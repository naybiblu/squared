...
export const actions = {
	default: async ({ request, cookies }: any) => {
		...
		const query = await users.find({
			$or: [{ 'credentials.email': email }, { username: un }]
		});
		await errorLogger(query[0]?.username ?? 'none');
		const emailCheck = query.map((q) => q.credentials.email).includes(email) ? ' email' : '';
		const unCheck = query.map((q) => q.username.toLowerCase()).includes(un.toLowerCase())
			? ' username'
			: '';
		...
		if (query.length !== 0)
			return fail(469, {
				error: `The${emailCheck} ${linker ? 'and' : ''}${unCheck} ${
					linker ? 'are' : 'is'
				} already taken.`
			});
