	...
	>
		...
		<form class="flex flex-col justify-center items-center gap-5" method="POST">
			<div class="flex justify-center items-center gap-3">
				<input
					type="text"
					class="rounded-lg p-2 bg-slate-200 text-center text-5xl
          h-20 w-16 sm:w-72"
					name="first"
					maxlength="1"
					bind:value={code.first}
					on:input={moveToNext}
				/>
				<input
					type="text"
          ...
          h-20 w-16 sm:w-72"
					name="fifth"
					maxlength="1"
					bind:value={code.fifth}
					on:input={moveToButton}
				/>
			</div>
			<button
				type={Object.values(code).includes('') ? 'button' : 'submit'}
