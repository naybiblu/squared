<script lang="ts">
	...
	let showPassword1: boolean, showPassword2: boolean, check: boolean;
	let data = {
		bday: '',
		pass1: '',
		pass2: '',
	...
	function passValidator() {
		if (data.pass1 && data.pass2) {
			...
			if (data.pass1 !== data.pass2)
				return (error.pass2 = 'Please retype your password correctly.');
			error.pass1 = '';
			error.pass2 = '';
		}
	}
...
>
	<div
		class="h-full w-1/3 backdrop-brightness-50 flex flex-col 
    justify-center items-center sm:w-full text-white"
	>
		<div
			style="transform: rotate(-90deg); font-size: 6rem;"
			class="flex flex-col 
      items-start gap-5"
		>
	...
	<div
		class="h-full w-2/3 bg-white flex justify-self-end
		...
		<div>
			<div class="flex gap-5 flex-wrap animate-fadeIn">
				<div class="flex-col flex gap-4 justify-center">
					...
					<div class="flex gap-2">
						<hr
							style="background: rgb(180 83 9); height: 0.75rem; width: 8rem;"
							class="rounded-full animate-pulse"
						/>
						<hr style="background: #D2D2D2; height: 0.75rem; width: 8rem;" class="rounded-full" />
					</div>
				</div>
			</div>
            ...
            h-10 w-96 sm:w-36 {!data.lastName ? 'outline-red-500' : ''}"
              ...
              h-10 w-[24.2rem] sm:w-36 {!data.bday ? 'outline-red-500' : ''}"
							name="bday"
							bind:value={data.bday}
							on:input={bdayValidator}
							required
						/>
					</div>
					<select
						...
						{#if error.pass1}
							<div class="absolute -top-[4.4rem] h-16 left-5 flex gap-3 items-end">
								<img
									src="../../img/warning.svg"
									alt="warning"
									class="h-6 animate-pulse peer/p1warn"
								/>
								<div
									class="bg-red-500 max-w-[19rem] p-2 rounded-lg text-white scale-0
            ...
            h-10 w-[21.3rem] {!data.pass1 || error.pass1 ? 'outline-red-500' : ''}"
							...
							name="passwordTrue"
							on:input={inputHandle1}
						/>
						<button
							class="rounded-r-full bg-slate-200 h-10 w-10 hover:bg-amber-700 flex
              justify-center items-center group/eye"
							on:click={togglePass1}
							type="button"
						>
							<img
								src="../../img/{showPassword1 ? 'eyehide' : 'eyeshow'}.svg"
								alt={showPassword1 ? 'show' : 'hide'}
            ...
            h-10 w-[21.3rem] {!data.pass1 || error.pass1 ? 'outline-red-500' : ''}"
							...
							name="password2"
							on:input={inputHandle2}
						/>
						<button
							class="rounded-r-full bg-slate-200 h-10 w-10 hover:bg-amber-700 flex
              justify-center items-center group/eye"
							on:click={togglePass2}
							type="button"
						>
							<img
								src="../../img/{showPassword2 ? 'eyehide' : 'eyeshow'}.svg"
								alt={showPassword2 ? 'show' : 'hide'}
								class="h-6 mr-1 group-hover/eye:invert"
							/>
						</button>
					</div>
          ...
          {!check
						...
						: 'hover:bg-slate-300 hover:text-black cursor-pointer'}"
					id="next">Next</button
				>
			</form>
		</div>
