<script lang="ts">
	...
	const hide = () =>
		setTimeout(() => document.getElementById('toggle')?.classList.add('hidden'), 100);
...
<div
	class="fixed top-0 h-16 w-screen m-0 z-10 justify-center items-center text-black
...
>
	<div class="flex justify-between items-center mb-1 w-full px-10">
		<div class="flex justify-start items-center flex-row gap-3">
			<form method="POST" action="?/search">
				<div class="flex flex-row">
					...
					<input
						type="text"
						class="bg-white h-10 w-60 rounded-r-full py-5
		...
		<div class="flex justify-between items-center w-7/12">
			<div class="flex justify-between items-center">
				...
				>
					<img
						src="img/feed.svg"
						alt="feed"
						class="h-10 group-hover/tt:scale-110 transition-all
                        {page === 1 ? 'a700' : 'invert'}"
					/>
					<span class="sidebar-tooltip group-hover/tt:scale-100 font-bold text-sm"> Feed </span>
				</a>
				<a
					href="/{user.username}"
					class="relative flex items-center 
				...
				<a
					href="/notes"
					class="relative flex items-center {page === 3 ? 'bg-white' : ''}
				...
				>
					...
					/>
					<span class="sidebar-tooltip group-hover/tt:scale-100 font-bold text-sm"> Notes </span>
				</a>
			</div>
			<div
				class="flex justify-start items-center flex-row bg-white 
                rounded-full px-2 py-2 mt-1 hover:bg-slate-100"
			>
				<button class=" group/nav rounded-full" on:click={toggleOptions} on:blur={hide}>
					<img
						src={user.avatar ?? defaultAv(`${user.fName} ${user.lName}`)}
						class="h-9 rounded-full"
						alt="profile"
					/>
					<span
						class="absolute bottom-3 right-12 p-[0.25px] bg-white rounded-full 
			...
			<div
				class="absolute right-10 top-20 flex-col bg-slate-200 shadow-sm
            text-sm z-10 rounded-md border-slate-300 border hidden divide-y 
            divide-gray-100 font-bold"
				id="toggle"
			>
				...
				<ul class="py-2 flex flex-col items-start justify-center w-full">
					{#each [{ name: 'Settings', link: '/settings' }, { name: 'Logout', form: true }] as list}
						{#if list.form}
							<li
								class="hover:bg-white w-full px-4 py-2 
