...
<div class="h-72 w-72 flex drop-shadow-md bg-yellow-100 relative group/post" id={post.id}>
	<div
		class="w-full h-9 px-3 flex absolute top-0 left-0 bg-yellow-200
        flex-col justify-center items-start"
	>
		<button class="absolute right-0 px-3 hover:scale-110" on:click={toggleOptions} on:blur={hide}>
	...
	<div
		class="absolute bottom-5 left-4 drop-shadow-md bg-white rounded-lg
        scale-0 hover:bg-blue-400 hover:invert group-hover/post:scale-100"
	>
		<button
			class="p-2 flex flex-row gap-2
	...
	<div
		class="absolute bottom-5 right-4 bg-transparent flex flex-row gap-2
	...
	>
		...
		<img
			class="h-10 rounded-lg drop-shadow-lg"
